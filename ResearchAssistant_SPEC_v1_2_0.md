# ResearchAssistant SPEC（目标架构 · v1.2.0）
...
- **架构级决策**（改动成本高、多模块依赖）记录在第 2 节"核心架构决策"。
...
- 跨文献综合问答(摘要为主 + 原文按需检索, 利用长上下文模型; 详见 ADR-1)
...
### ADR-1 · 跨文献分析采用"摘要为主 + 原文按需检索"的混合架构

**状态**: 已定（检索层 v1.2 落地, 数据模型 v1.1 即预留接口）

**决策**: 跨文献综合问答以**结构化摘要拼接**为主路径; 当用户的问题超出摘要 12 个字段能覆盖的范围时, 系统从 `paper_pages` 全文中**按需检索**相关片段补入上下文。检索采用**本地 embedding**（如 bge-small 类模型, 离线、免费、私密）。

**为什么这么定**:
- 纯摘要方案把全部价值压在"提取"这一步: 摘要是有损压缩, 提取漏掉的信息后续任何模型都无法找回, 这是单点风险。
- 纯原文 RAG 方案对 20-50 篇文献成本与延迟过高, 且与"便宜模型"定位冲突。
- 混合方案让摘要承担 80% 的常见问答（快、省 token, 20 篇摘要约 15K-25K tokens）, 检索只在摘要答不上来时兜底, 既稳又省。本地 embedding 与"绝对私密"定位一致。

**实施分期**:
- **v1.1（基线）**: 仅摘要拼接路径上线; 但 `paper_pages` 必须保留完整全文, 数据模型为检索预留接口（不删页文本、保留 `char_count`）。
- **v1.2（增强）**: 接入本地 embedding, 对 `paper_pages` 分块建向量索引, 跨文献问答与单篇精读均支持"摘要答不出 → 回原文检索"。

**对其他模块的约束**:
- `paper_pages` 是检索的数据基础, **任何"省空间删全文"的优化都被禁止**。
- 跨文献问答 API（§6.4）的响应契约需为"检索补充上下文"预留, 不得假设上下文只来自摘要。
...
|    + 后台提取 Job worker + 本地 embedding 检索(v1.2)      |
+---------------------------------------------+
```
...
- **上下文构建（见 ADR-1）**: 默认仅拼接所选文献的结构化摘要; v1.2 起, 当问题超出摘要字段覆盖范围时, 从 `paper_pages` 按需检索相关片段补入上下文。响应契约不假设上下文只来自摘要。
...
**Phase 4 · 能力增强（v1.2）**
- [ ] 本地 embedding 检索层（ADR-1）: 语义搜索 + 单篇/跨文献"摘要答不出回原文"
...
- [ ] **ADR-1（v1.2）**: 问"摘要未覆盖"的问题时, 系统能回原文检索补充并正确作答
- [ ] **ADR-5（v1.2）**: 提取/问答后 token_usage 有记录, 项目维度可汇总成本
