...
2. **模型只做召回辅助, 不做实质裁决**(受 ADR-7 约束): 对每个候选, 模型只执行"找出表面相似/可能相关的候选组", 一次只看极小输入做分组辅助。可并行、可按 hash 缓存、增量计算。实质共性判断**严禁**由模型完成——上移到人的裁决面板。
...
> **注意**: ADR-6 的"聚焦裁决"步骤在 ADR-7 下被重新定义——模型的职责从"判定两个论断是否为同一局限"收窄为"找出表面相似的候选组", 实质共性判断一律留给人的裁决面板。详见 ADR-7。

### ADR-7 · 信号呈现铁律: AI 召回、人裁决, AI 绝不替人做"实质共性"判断

**状态**: 已定

**决策**: 所有机会信号(局限聚类 / 矛盾检测 / 方法迁移等)的输出形态, 不是**"AI 断言的结论"**(如"这 N 篇有共同局限 X"), 而是**"AI 召回的候选 + 判断所需的完整依据"**, 由人裁决该候选是否成立。

**为什么这么定**: 跨文献的实质共性判断依赖领域实质知识(生物学对象、实验条件、机制层次等), LLM 会被语言表面相似系统性误导。实例: Buchholz 的 RPE65 表达问题与 Iwama 的 PR/BP 分化问题, 都含"不明确"措辞, LLM 可能将它们归为一簇——但生物学对象完全不同(RPE 上皮 vs 光感受器神经元), 是两件本质不同的事。自动化程度越高, 这类"听似合理、实质错误"的输出越危险, 且越难被使用者察觉——因为措辞是通的、结论是可读的, 但基础事实是错的。

**设计推论（三条硬约束）**:
1. **召回宁可宽(Never miss)**。少摆候选是系统失职, 多摆让人的注意力去过滤是设计意图。AI 的召回职责是"别漏", 不是"判准"。
2. **裁决必须由人(The human adjudicates)**。判准是人的职责——人拥有领域知识、能识别"这两条不是一回事"。系统决不替人做此判断, 也不以置信度/信号名暗示此判断已完成。
3. **核心工作不是"提高聚类准确率"(徒劳), 而是"把判断依据摆全"**。每条候选必须标注: 来源文献、研究对象(RPE / PR / iPSC 等)、原文出处(逐字 quote + 页码)、topic、分组方式(确定性/embedding 相似度/LLM 辅助), 使人能一眼识别"这两条其实不是一回事"。

**对 ADR-6 的修正**: ADR-6 第二条"聚焦模型逐对裁决"中的"裁决"被本 ADR 重新定义——模型不再执行"判定两个论断是否属于同一局限"的实质判断, 而是执行"找出表面相似/可能相关的候选组"的召回辅助。实质共性判断上移到人的裁决面板。

**对其他模块的约束**:
- 信号面板(§4.2)的 UI 必须包含**显式裁决区**(接受/否决/改判/批注), 而非仅展示 AI 结论卡片。
- 信号输出格式**禁止**包含 AI 生成的"信号名"作为最终结论、"置信度"暗示判断已完成; 只可有"候选分组标签"和"分组依据说明"。
- 所有信号流水线的输出结构统一为 `(候选组, 依据清单)` 而非 `(信号名, 置信度, 理由)`, 确保"换一个模型/embedding 不会悄悄改变裁决语义"——裁决只发生在人点下按钮的那一刻。
...
- **信号面板**: 自动 surface 四类候选信号(局限聚类 / 矛盾点 / 稀疏格 / 方法迁移), 每条做成可追溯的**裁决卡片**(论断 + 逐字原句 + 页码 + 研究对象 + topic + 分组依据), 由人裁决是否成立。**AI 不给结论**(见 ADR-7), 只摆候选+依据。
...
> 不一次全建。以"一个信号端到端立起一条纵切片"为节奏, 每个切片打通: claims 字段 → 抽取 → 确定性召回 → 聚焦裁决(模型辅助分组, 见 ADR-7) → 可追溯裁决 UI(人 accept/reject, ADR-7) → 喂给立项导出。
...
| **AI 表面相似误导→信号伪阳性增大** | **高** | **ADR-7 铁律: AI 不替人做实质共性判断; 信号面板只摆候选+依据, 由人裁决。伪阳性交给人的注意力去过滤, 危害远小于静默漏过真实信号** |
...
- [ ] **ADR-7**: 信号面板不出现 AI 断言的"信号名"或"置信度"作为最终结论; 每条候选展示的是分组依据+完整证据(研究对象/出处/topic), 人通过显式裁决区(接受/否决/改判/批注)完成实质判断
